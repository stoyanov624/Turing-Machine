...
WhileComposedTM::WhileComposedTM(const TuringMachine& _decider, const TuringMachine& _tm) {
	machine_ID_generator -= 3;
	machine_ID = machine_ID_generator++;
	decider = _decider;
	tm = _tm;
...
void WhileComposedTM::saveResult() const {
	std::string result_file_str = "results_from_While_machines\\result" + std::to_string(machine_ID) + ".txt";
